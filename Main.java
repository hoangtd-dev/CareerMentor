...
public class Main {
    ...
    public static int getAdjacentValues(int[] arr) {
        ...
        while (first < arr.length - 1) {
            if (second == arr.length || (first != second - 1 && arr[second - 1] != arr[first] && arr[second - 1] != arr[second])) {
                first++;
                second = first + 1;
                continue;
            }
    ...
    public static int getDistanceOfAdjacentValues(int[] arr) {
        ...
        for (int i = 0; i < arr.length - 1; i++) {
            ...
            int maxDistanceWithFirstNum = Math.max(Math.abs(firstIndexes[0] - secondIndexes[0]), Math.abs(firstIndexes[0] - secondIndexes[1]));
            int maxDistanceWithSecondNum = Math.max(Math.abs(firstIndexes[1] - secondIndexes[0]), Math.abs(firstIndexes[1] - secondIndexes[1]));
